...
const FALLBACK_TRANSLATION_LANGUAGES: LanguageOption[] = [
...
export function getFallbackTranslationLanguages() {
  return [...FALLBACK_TRANSLATION_LANGUAGES].sort((left, right) => left.name.localeCompare(right.name));
}

export async function getGoogleApiErrorMessage(response: Response, fallbackMessage: string) {
  ...
  if (!upstreamMessage) {
    return `${fallbackMessage} Google returned HTTP ${response.status}.`;
  }

  return `${fallbackMessage} ${upstreamMessage}`;
}
